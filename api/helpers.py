...
def generate_slug(text,x):
    ...
    if x == "anime":
        if AzItemAnime.objects.filter(slug = new_slug).first():
            return generate_slug(text + generate_random_string(5))
    elif x == "sub_anime":
        if AzSubItemAnime.objects.filter(slug = new_slug).first():
            return generate_slug(text + generate_random_string(5))
    elif x == "movie":
        if AzItemMovie.objects.filter(slug = new_slug).first():
            return generate_slug(text + generate_random_string(5))
    return new_slug
